...
object KinesisWordCount {
  def main(args: Array[String]){
    ...
    if (args.length < 3) {
      System.err.println("Usage: KinesisWordCount <host> <stream_name> <kinesis_endpoint>")
      System.exit(1)
    }

    val Array(host, streamName, endpointUrl) = args
    ...
    words.foreachRDD(foreachFunc = (rdd: RDD[String], time: Time) => {
      ...
      val jsonrdd = sqlContext.jsonRDD(rdd).registerTempTable("words")

      val wordCountsDataFrame =
        sqlContext.sql("select level, count(*) as total from words group by level")
      println(s"========= $time =========")
